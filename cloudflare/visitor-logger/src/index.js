...
async function sendEmailAlert(visitData, env) {
  ...
  const subject = `🚨 Target Visitor Alert (matferg.com): ${visitData.city}, ${visitData.region} (${visitData.postalCode})`;
  const bodyText = `A visitor from your target area just visited matferg.com!
  ...
  if (env.RESEND_API_KEY) {
    try {
      await fetch("https://api.resend.com/emails", {
        ...
        body: JSON.stringify({
          from: "matferg.com <onboarding@resend.dev>",
          to: [recipient],
          subject: subject,
          text: bodyText,
        }),
  ...
  try {
    await fetch("https://api.mailchannels.net/tx/v1/send", {
      ...
      body: JSON.stringify({
        ...
        from: { email: "alerts@matferg.com", name: "matferg.com Visitor Alert" },
        subject: subject,
...
async function sendDailyDigest(env, siteName = "matferg.com") {
  const recipient = "mcferguson9@gmail.com";
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (url.searchParams.get("test_alert") === "1") {
      ...
      return new Response("Test alert email dispatched for matferg.com to mcferguson9@gmail.com!");
    }

    if (url.searchParams.get("test_digest") === "1") {
      await sendDailyDigest(env, "matferg.com");
      return new Response("Daily ZIP code digest dispatched for matferg.com to mcferguson9@gmail.com!");
    }
